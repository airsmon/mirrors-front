...
- 道客镜像： https://mirrors.idc.daocloud.io/CTAN/
...
TeX Live 发行版的常见安装方法可以参考[此文档](https://mirrors.idc.daocloud.io/CTAN/info/install-latex-guide-zh-cn/install-latex-guide-zh-cn.pdf)。

除每年更新的完整版 ISO 镜像以外， CTAN 镜像中也包含在线安装器。这种方法可以使安装的所有宏包均为最新版本，但受网络连接状况影响较大。操作方法为（*很可能需要管理员权限*）：

1. 下载 [`install-tl.zip`](https://mirrors.idc.daocloud.io/CTAN/systems/texlive/tlnet/install-tl.zip) 并解压缩
2. Windows 下双击运行其中的 `install-tl.bat` 。如果有图形化界面，可以在进入安装器前的右下角按钮指定使用镜像源。
Linux 下使用如下命令：
```bash
perl install-tl --repository https://mirrors.idc.daocloud.io/CTAN/systems/texlive/tlnet
```
...
```bash
tlmgr option repository https://mirrors.idc.daocloud.io/CTAN/systems/texlive/tlnet
```
...
```bash
tlmgr update --all --repository https://mirrors.idc.daocloud.io/CTAN/systems/texlive/tlnet
```
...
```bash
mpm --set-repository=https://mirrors.idc.daocloud.io/CTAN/systems/win32/miktex/tm/packages/
```
